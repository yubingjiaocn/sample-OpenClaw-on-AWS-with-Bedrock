...
variable "enable_admin_console" {
  description = "Enable Admin Console deployment on EKS (DynamoDB, S3, ECR, Pod Identity)"
  type        = bool
  default     = false
}

variable "admin_password" {
  description = "Admin console login password (required when enable_admin_console = true)"
  type        = string
  default     = ""
  sensitive   = true
}
